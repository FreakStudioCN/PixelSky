...
const COLORS = { bg: "#000000", night: "#24103D", mint: "#31F5C3", mintDark: "#149C82", purple: "#9B7BFF", pink: "#FF5A9D", yellow: "#FFCB5C", blue: "#52B7FF", white: "#FFFFFF" };
...
async function remoteFrames(env: Env, prompt: string, width: number, height: number): Promise<RemoteResult> {
  ...
  const subjectGuidance = prompt.includes("猫") || lowPrompt.includes("cat")
    ...
    : prompt.includes("机器人") || lowPrompt.includes("robot")
      ? "The robot must have a square mechanical head or body, two separated glowing eyes, a clear hat above the head, two arms, and two legs when space permits. For waving, keep one arm fixed and move the other arm between clearly different raised positions."
      : prompt.includes("流星") || prompt.includes("彗星") || lowPrompt.includes("meteor") || lowPrompt.includes("comet")
        ? "Draw one meteor only: a compact bright 2x2 or diamond-shaped head and one tapered tail extending behind it. Never draw an X, two symmetric diagonal lines, or two meteors. Move the same head and tail consistently between frames while keeping every frame visible."
        : "Use the subject's most distinctive silhouette and at least three identifying features so it is recognizable without explanation.";
  try {
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      body: JSON.stringify({
        ...
        stream: false,
        messages: [
          { role: "system", content: `You are a professional pixel-art animator for a physical LED matrix. Understand Simplified Chinese instructions as the primary input language. Create the exact requested subject, colors, setting, and action. Never substitute an unrelated subject or return blank/solid-color frames. The canvas is exactly ${width}x${height}; use hard pixel edges, a centered readable silhouette, no antialiasing, and a limited high-contrast palette. Preserve the same subject between frames and change only the requested motion. In the JSON, complete the design object before drawing frames so the pixels follow a deliberate silhouette, feature, color, and motion plan. Return one valid JSON object with design, palette, and frames, and no Markdown.` },
          { role: "user", content: JSON.stringify({ user_request: prompt, selected_canvas: `${width}x${height}`, frame_count: `2 to ${maxGeneratedFrames}`, recognition_guidance: subjectGuidance, retry_instruction: attempt ? "The previous result was blank or lacked a visible subject. Redesign it with a clear foreground silhouette and distinctive requested features." : undefined, required_design_object: { subject: "exact requested subject", distinctive_features: ["at least three visible identifying features"], color_plan: "map every requested color to a role", motion_plan: "state which pixels move while identity stays fixed" }, structural_example_do_not_copy: { palette: ["#000000", "#FFFFFF"], frames: [exampleRows] }, output_rules: [`palette must be a JSON array of 2-10 #RRGGBB strings`, `frames must be an array of frames`, `each frame must contain exactly ${height} row strings`, `each row string must contain exactly ${width} palette-index digits`, "palette index 0 is the background", "use at least two palette indices in every frame"], requirements: ["Match the user's subject and requested colors instead of copying the structural example", "Use the completed design plan when drawing every frame", "Foreground must occupy enough pixels to be clearly visible", "Include the subject's distinctive features", "Keep every frame inside the selected canvas", "Use the same palette for all frames"] }) },
        ],
      }),
      });
      if (!response.ok) {
