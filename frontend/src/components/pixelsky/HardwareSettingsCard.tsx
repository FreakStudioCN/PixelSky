...
import { BOARD_PROFILES, boardDefaultPin, type BoardProfile, type MatrixLayout, type PixelOrder, type PixelProject } from "@/lib/pixel";
...
export function HardwareSettingsCard({ project, onChange }: HardwareSettingsCardProps) {
  ...
  return <section className="panel p-4">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      <label className="grid gap-1 text-[10px] text-muted-foreground">开发板<select value={project.board} onChange={(event) => { const board = event.target.value as BoardProfile; onChange({ board, pin: boardDefaultPin(board) }); }} className="h-9 rounded-md border border-input bg-background px-3 text-xs text-foreground">{BOARD_PROFILES.map((profile) => <option key={profile.id} value={profile.id}>{profile.label}</option>)}</select></label>
