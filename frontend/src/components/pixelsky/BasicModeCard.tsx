...
export function BasicModeCard({ display, colors, temperature, weather, locationLabel, loading, error, brightness, autoSync, syncDisabled, syncing, onDisplayChange, onColorsChange, onRefresh, onBrightnessChange, onAutoSyncChange }: BasicModeCardProps) {
  ...
  return <section className="panel p-4">
    ...
    <div className="mt-4 flex items-center gap-2 text-xs font-medium"><Palette className="h-4 w-4 text-primary" />{display === "weather" ? "天气图案颜色已锁定" : "文字颜色"}</div>
    {display === "weather" ? <p className="mt-2 rounded-md border border-border bg-background px-3 py-2 text-[10px] leading-5 text-muted-foreground">天气颜色不可修改：云和雪为白色、雨为蓝色、闪电为黄色。</p> : display === "time" ? <div className="mt-2 grid grid-cols-2 gap-2">{colorField("hour", "小时颜色")}{colorField("minute", "分钟颜色")}</div> : <div className="mt-2">{colorField("text", "文字颜色")}</div>}
    <div className="mt-4 rounded-lg border border-border bg-background p-3">
      <div className="flex items-center justify-between"><span className="text-xs font-medium">输出亮度</span><span className="font-mono text-xs text-primary">{brightness}%</span></div>
      <Slider className="mt-3" min={1} max={100} step={1} value={[brightness]} onValueChange={(value) => onBrightnessChange(value[0] ?? brightness)} />
      <p className="mt-2 text-[10px] leading-5 text-muted-foreground">为保护灯板和供电，100% 对应设备端安全亮度上限。</p>
    </div>
