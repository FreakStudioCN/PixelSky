...
export function BasicModeCard({ display, colors, temperature, weather, locationLabel, loading, error, autoSync, syncDisabled, syncing, onDisplayChange, onColorsChange, onRefresh, onAutoSyncChange }: BasicModeCardProps) {
  const textColorField = <label className="grid gap-1 text-[10px] text-muted-foreground">
    <span>文字颜色</span><span className="flex h-9 items-center gap-2 rounded-md border border-input bg-background px-2"><input type="color" value={colors.text} onChange={(event) => onColorsChange({ text: event.target.value.toUpperCase() })} className="h-6 w-8 cursor-pointer border-0 bg-transparent p-0" /><span className="font-mono text-[10px] text-foreground">{colors.text}</span></span>
  </label>;
  return <section className="panel p-4">
    ...
    {display === "weather" ? <p className="mt-2 rounded-md border border-border bg-background px-3 py-2 text-[10px] leading-5 text-muted-foreground">天气颜色不可修改：云和雪为白色、雨为蓝色、闪电为黄色。</p> : <div className="mt-2">{textColorField}</div>}
